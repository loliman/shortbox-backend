	...
	if len(i.Stories) != 0 {
		...
		for _, newStory := range i.Stories {
			...
			if !found {
				...
				//Issue OI
				res, err = db.Exec("INSERT INTO Issue (title, fk_series, number, format, language, pages, releasedate, price, currency, coverurl, quality, qualityAdditional, amount, isread, originalissue) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", newStory.OriginalIssue.Title, newStory.OriginalIssue.Series.Id, newStory.OriginalIssue.Number, newStory.OriginalIssue.Format.Format, newStory.OriginalIssue.Language, newStory.OriginalIssue.Pages, newStory.OriginalIssue.Releasedate, newStory.OriginalIssue.Price.Price, newStory.OriginalIssue.Price.Currency, newStory.OriginalIssue.Coverurl, newStory.OriginalIssue.Quality, newStory.OriginalIssue.QualityAdditional, newStory.OriginalIssue.Amount, newStory.OriginalIssue.Read, newStory.OriginalIssue.Originalissue)

				if err != nil {
					err = db.QueryRow("SELECT * FROM Issue WHERE fk_series = ? AND number = ?", newStory.OriginalIssue.Series.Id, newStory.OriginalIssue.Number).Scan(&newStory.OriginalIssue.Id, &newStory.OriginalIssue.Title, &newStory.OriginalIssue.Series.Id, &newStory.OriginalIssue.Number, &newStory.OriginalIssue.Format.Format, &newStory.OriginalIssue.Format.Variant, &newStory.OriginalIssue.Language, &newStory.OriginalIssue.Pages, &newStory.OriginalIssue.Releasedate, &newStory.OriginalIssue.Price.Price, &newStory.OriginalIssue.Price.Currency, &newStory.OriginalIssue.Coverurl, &newStory.OriginalIssue.Quality, &newStory.OriginalIssue.QualityAdditional, &newStory.OriginalIssue.Amount, &newStory.OriginalIssue.Read, &newStory.OriginalIssue.Originalissue)
